...
- “智能自适应”使用极验控制台为当前应用配置的验证组合，由极验策略决定本次交互形态，建议首次部署使用此模式。
- “一点即过”“滑动拼图”“消消乐”“五子棋”“九宫格”“文字点选”“语序点选”和“图标点选”属于服务端签名的指定形态。使用前必须在极验控制台为当前应用启用“风控融合模式”并开通对应形态。
- 如果控制台仍是智能模式却在插件中强制指定形态，极验可能拒绝 `riskType`，表现为初始化失败、行为验证码错误或 `-50001`。
- “一点即过”本身不提供图形交互拦截能力，不建议单独用于高风险场景。
...
- 登录、注册、重置密码、修改联系方式使用“智能自适应”或已在极验控制台启用的交互挑战。
- 不建议在高风险业务中固定使用“一点即过”。
